...
{
   public class ComputerPlayerLogic2 : IPlayerMovements
    {
        int numberOfMarkedPossition = 1;
        ...
        private void GenerateNewPossition(Terrain currentTerrain)
        {
            for (int row = 0; row < currentTerrain.SizeOfTheMatrix; row++)
            {
                for (int col = 0; col < currentTerrain.SizeOfTheMatrix; col++)
                {
                    ...
                    {
                         if (col < currentTerrain.SizeOfTheMatrix - 1 && currentTerrain.TerrainMatrix[row, col + 1] == 0)
                        {
                        ...
                        else if (row > 0 && currentTerrain.TerrainMatrix[row - 1, col] == 0)
                        {
